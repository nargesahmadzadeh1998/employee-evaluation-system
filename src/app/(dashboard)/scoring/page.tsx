...
export default function ScoringPage() {
  ...
  useEffect(() => {
    if (selectedDept) {
      fetch(`/api/scores?departmentId=${selectedDept}`)
        .then((r) => r.json())
        .then(setData);
    }
  }, [selectedDept]);
  ...
  const getSkillsForJobTitle = (jobTitle: string) => {
    ...
    return data.skills.filter((s: any) =>
      s.jobTitleLinks.some((jt: any) => jt.jobTitle === jobTitle)
    );
  };

  const getSkillAverage = (employee: any, skill: any): number | null => {
    const criteriaIds = skill.criteria.map((c: any) => c.id);
    const relevantScores = employee.scores.filter(
      (s: any) => criteriaIds.includes(s.criterionId) && s.value !== null
    );
  ...
  const handleSaveScores = async () => {
    setSaving(true);
    const scores = Object.entries(criteriaScores).map(([criterionId, value]) => ({
      criterionId,
      value,
    }));
    ...
    await fetch("/api/scores", {
      ...
      body: JSON.stringify({ employeeId: modalEmployee.id, scores }),
    });
  ...
  // Get all unique skills across employees in this department
  const allSkills = data?.employees
    ? Array.from(new Set(data.employees.flatMap((e: any) => getSkillsForJobTitle(e.jobTitle).map((s: any) => s.id))))
        .map((id) => data.skills.find((s: any) => s.id === id))
        .filter(Boolean)
    : [];
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">Evaluate employees by skill criteria</p>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase sticky left-0 bg-gray-50 min-w-[180px]">Employee</th>
                <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase min-w-[100px]">Job Title</th>
                {allSkills.map((skill: any) => (
                  <th key={skill.id} className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase min-w-[120px]">
                    {skill.name}
                  </th>
                ))}
                <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase min-w-[100px]">Overall</th>
                <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase min-w-[160px]">Suggestion</th>
              </tr>
            </thead>
            <tbody>
              {data.employees.map((emp: any) => {
                ...
                const suggestion = getSuggestion(overall);
                const bgClass = getSuggestionBg(overall);

                return (
                  <tr key={emp.id} className={`border-b border-gray-100 ${bgClass}`}>
                    <td className="px-4 py-3 text-sm font-medium text-gray-900 sticky left-0"
                    ...
                    {allSkills.map((skill: any) => {
                      ...
                      if (!hasSkill) {
                        return <td key={skill.id} className="px-4 py-3 text-center text-gray-300 text-sm">—</td>;
                      }
                      const avg = getSkillAverage(emp, skill);
                      return (
                        <td key={skill.id} className="px-4 py-3 text-center">
                          <button
                            onClick={() => openScoring(emp, skill)}
                            className="inline-flex items-center justify-center w-full px-3 py-1.5 rounded-lg text-sm font-medium transition hover:ring-2 hover:ring-blue-300 bg-white border border-gray-200"
                          >
                            {avg !== null ? avg.toFixed(1) : "Score"}
                          </button>
                        </td>
                      );
                    })}
      ...
      <Modal
        ...
        title={`${modalSkill?.name} — ${modalEmployee?.name}`}
        wide
      >
        {modalSkill && (
          <div className="space-y-4">
            {modalSkill.description && (
              <p className="text-sm text-gray-500 mb-4">{modalSkill.description}</p>
            )}
            {modalSkill.criteria.map((criterion: any) => (
              <div key={criterion.id} className="flex items-center justify-between gap-4 py-2 border-b border-gray-100">
                ...
                <select
                  value={criteriaScores[criterion.id] === null ? "na" : criteriaScores[criterion.id] ?? "na"}
                  onChange={(e) => {
                    const val = e.target.value === "na" ? null : parseInt(e.target.value);
                    setCriteriaScores({ ...criteriaScores, [criterion.id]: val });
                  }}
            ...
            <button
              ...
              disabled={saving}
              className="w-full bg-blue-600 text-white py-2.5 rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50 transition mt-4"
            >
